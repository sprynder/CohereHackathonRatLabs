...
test_data_plane_index = index_fixture_factory(
    [(RemoteIndex(pods=2, index_name=f'{INDEX_NAME_PREFIX}-{PodType.P1}',
     ...
     (RemoteIndex(pods=2, index_name=f'{INDEX_NAME_PREFIX}-{PodType.P2}', dimension=vector_dim,
...
def get_test_data_dict(vector_count=10, no_meta_vector_count=5, dimension=vector_dim):
    return {id: (values, metadata) for id, values, metadata in get_test_data(vector_count, no_meta_vector_count, dimension)}
...
def async_upsert(index, namespace, data, batch_size):
    ...
    async_results = [index.upsert(vectors=(
        [
            Vector(id=id_, values=values, metadata=metadata) if metadata
            else Vector(id=id_, values=values)
            for id_, values, metadata in data_chunk
        ]
    ),
        namespace=namespace,
        async_req=True
...
def test_summarize_no_api_key():
    ...
    with pytest.raises(ApiKeyError) as exc_info:
        nonexistent_index = Index('nonexistent-index')
...
def test_summarize_nonexistent_index(pinecone_init):
    ...
    with pytest.raises(PineconeProtocolError) as exc_info:
        nonexistent_index = Index('nonexistent-index')
        api_response = nonexistent_index.describe_index_stats()
        logger.debug('got api response {}', api_response)
    logger.debug('got expected exception: {}', exc_info.value)
...
def test_invalid_upsert_vector_no_values(test_data_plane_index):
    index, _ = test_data_plane_index
    with pytest.raises(TypeError) as exc_info:
        api_response = index.upsert(vectors=[Vector(id='bad_vec1')])
        logger.debug('got api response {}', api_response)
    logger.debug('got expected exception: {}', exc_info.value)
    assert "missing 1 required positional argument: 'values'" in str(exc_info.value)
...
def test_upsert_vectors_no_metadata(test_data_plane_index):
    ...
    api_response = index.upsert(
        vectors=[
            ...
            Vector(id='vec2', values=[0.2] * vector_dim),
        ],
        namespace=namespace,
    )
...
def test_upsert_vectors(test_data_plane_index):
    ...
    api_response = index.upsert(
        vectors=[
            Vector(id='mvec1', values=[0.1] * vector_dim, metadata={'genre': 'action', 'year': 2020}),
            Vector(id='mvec2', values=[0.2] * vector_dim, metadata={'genre': 'documentary', 'year': 2021}),
        ],
        namespace=namespace,
    )
    assert api_response == UpsertResponse(upserted_count=2)
...
def test_invalid_upsert_vectors_wrong_dimension(test_data_plane_index):
    ...
    with pytest.raises(ApiException) as exc_info:
        api_response = index.upsert(
            vectors=[
                Vector(id='vec1', values=[0.1] * 50, metadata={}),
                Vector(id='vec2', values=[0.2] * 50, metadata={}),
            ],
            namespace='ns1',
        )
        logger.debug('got api response {}', api_response)
...
def test_fetch_vectors_no_metadata(test_data_plane_index):
    ...
    assert not api_response.vectors.get('vec1').get('metadata')
...
def test_invalid_fetch_nonexistent_namespace(test_data_plane_index):
    ...
    assert len(api_response.vectors.keys()) == 0
    logger.debug('got openapi fetch response: {}', api_response)
...
def test_query_simple_with_values_mixed_metadata(test_data_plane_index):
    ...
    api_response = index.query(
        queries=[
            QueryVector(values=[0.1] * vector_dim),
            QueryVector(values=[0.2] * vector_dim),
        ],
        namespace=namespace,
        top_k=top_k,
        include_values=True,
...
def test_query_with_per_vector_top_k(test_data_plane_index):
    ...
    api_response = index.query(
        queries=[
            QueryVector(values=[0.1] * vector_dim, filter={}, top_k=5),
            QueryVector(values=[0.2] * vector_dim, filter={}),
        ],
        namespace=namespace,
        top_k=10,
        include_values=True,
...
def test_query_by_id_not_found(test_data_plane_index):
    ...
    assert f"Could not find vector with id '{id_}' in namespace '{namespace}'" in exc_info.value.body
...
def test_query_with_exists_filter(test_data_plane_index):
    ...
    count_before = get_vector_count(index, namespace)
    vectors = [('0', [0.1] * vector_dim, {'colors': True, 'country': 'greece'}),
               ('1', [-0.1] * vector_dim, {'colors': False}),
               ('2', [0.2] * vector_dim, {})]
    upsert_response = index.upsert(vectors=vectors, namespace=namespace)
    retry_assert(lambda: len(vectors) == get_vector_count(index, namespace) - count_before)
    logger.debug('got upsert response: {}', upsert_response)

    response_1 = index.query(vector=[0.1] * vector_dim,
                             filter={'$or': [{'colors': {'$exists': False}}, {'country': {'$eq': 'greece'}}]},
                             top_k=10,
                             namespace=namespace)
    logger.debug('got query response: {}', response_1)

    matches_ids = {match_vector.id for match_vector in response_1.matches}
    assert len(response_1.matches) == 2
    assert matches_ids == {'0', '2'}

    response_2 = index.query(vector=[0.1] * vector_dim,
                             filter={'colors': {'$exists': True}},
                             top_k=10,
                             namespace=namespace)
    logger.debug('got query response: {}', response_2)

    matches_ids = {match_vector.id for match_vector in response_2.matches}
    assert len(response_2.matches) == 2
...
def test_scrape(test_data_plane_index):
    index, index_name = test_data_plane_index
    ...
    def _scrape():
        ...
        r.raise_for_status()
        metrics = set()
        for metric_family in list(text_string_to_metric_families(r.text)):
            if metric_family.samples[-1].labels['index_name'] == index_name:
                metrics.add(metric_family.name)
                assert metrics_names[metric_family.name](metric_family.samples[-1])
        return metrics

    def valid_labels(labels, request_type=False, status_code=False):
        expected_labels = {'index_name': index_name,
                           'project_name': 'test'}
        request_types = {'upsert', 'describe_index_stats', 'query', 'delete', 'fetch', 'update'}
        valid_request_type = labels.pop('request_type') in request_types if request_type else True
        has_status_code = labels.pop('status_code') if status_code else True
        return labels.pop('shard') != '' and labels.pop(
            'replica') != '' and valid_request_type and has_status_code and labels == expected_labels

    for i in range(10):
        with pytest.raises(ApiException):
            index.upsert(vectors=[Vector(id=f'bad_vec{i}', values=[0.1, 0.2])])
    write_test_data(index, '')
    stats = index.describe_index_stats()
    metrics_names = {'pinecone_request_count': lambda x: x.value > 0 and valid_labels(x.labels, request_type=True),
                     'pinecone_vector_count': lambda x: x.value > 0 and valid_labels(x.labels),
                     'pinecone_index_fullness': lambda x: abs(
                         stats.index_fullness - x.value) < 1e-3 and valid_labels(
                         x.labels),
                     'pinecone_request_error_count': lambda x: x.value > 0 and valid_labels(x.labels,
                                                                                            request_type=True,
                                                                                            status_code=True),
                     'pinecone_request_latency_seconds': lambda x: x.value > 0 and valid_labels(x.labels,
                                                                                                request_type=True)}

    retry_assert(lambda: len(_scrape()) == len(metrics_names))
